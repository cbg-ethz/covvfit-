...
def fitted_values(ts_lst, model_map_fixed, cities):
    ...
    y_fit_lst = [softmax_1(ts_lst[i], model_map_fixed['rate'], model_map_fixed['midpoint'][i,:]) for i, city in enumerate(cities)]
...
def pred_values(ts_lst, model_map_fixed, cities, horizon=60):
    ...
    y_pred_lst = [softmax_1(ts_pred_lst[i], model_map_fixed['rate'], model_map_fixed['midpoint'][i,:]) for i, city in enumerate(cities)]
...
def compute_overdispersion(ys_lst, y_fit_lst, cities):
    ...
    pearson_r_lst = [(y_fit_lst[k] - ys_lst[k])**2 / (y_fit_lst[k]*(1-y_fit_lst[k])) for k, city in enumerate(cities)]
    overdisp_list2 = [i.sum() / ((i.shape[0])*(i.shape[1]-1)) for i in pearson_r_lst]
    overdisp_fixed = np.concatenate(pearson_r_lst, axis=1).sum() / np.sum([((i.shape[0])*(i.shape[1]-1)) for i in pearson_r_lst])
...
def make_ratediff_confints(t_rate, model_hessian_fixed, overdisp_fixed=1.0, g=7.0):
    ...
    rate_diff_se = np.sqrt(np.array([[t_hess_inv[[i,j],:][:,[i,j]].diagonal().sum() -  np.fliplr(t_hess_inv[[i,j],:][:,[i,j]]).diagonal().sum() for i,_ in enumerate(t_rate)] for j,_ in enumerate(t_rate)]))
...
def make_fitness_confints(t_rate, model_hessian_fixed, overdisp_fixed=1.0, g=7.0):
    ...
    for i,r1 in enumerate(t_rate):
        ...
        for j,r2 in enumerate(t_rate):
            t_jacobian = np.array([g*np.exp(g*(r2-r1)), -g*np.exp(g*(r2-r1))])
            se = np.sqrt(t_jacobian.dot(t_hess_inv[[i,j],:][:,[i,j]]).dot(t_jacobian.T))
            t_lst.append(se)
