...
for k, (config, proc_data) in enumerate(zip(all_configs, all_data)):
    ...
    for i, (var_idx, var_name) in enumerate(
        ...
        for j, horizon in enumerate(horizons):
            ax.plot(
                max_dates,
                rel_diff_array.mean(axis=1)[:, horizon, var_idx],
                label=int(horizon),
                color=colors[j],
            )
        ax.set_ylabel("mean re. err.")
        ax.set_ylim((-1, 1))
        ax.set_title(f"{var_name} rel. pred. error")
        ax.tick_params(axis="x", rotation=0)
    ...
    for cutoff, newrow in zip([0.025, 0.05, 0.10], [3, 4, 5]):
        ...
        err_before = rel_diff_array[max_dates_idx < break_index, :, :, :]
        err_after = rel_diff_array[max_dates_idx >= break_index, :, :, :]
        ...
        ax.set_xlabel("horizon (days)")
        ax.set_title(f"abs. rel. pred. error before/after {cutoff*100}% cutoff")
        ax.set_ylim([0, 4])
